      ...
      <div className="ml-64 px-10 py-10">
        <div className="mb-8 flex items-end justify-between">
          <div>
            ...
            <p className="text-sm text-white/60 mt-1">Admins manage agents. Agency owners manage themes.</p>
          </div>
          ...
          <div className="flex gap-2">
            {(['profile', 'agents', 'positions', 'themes'] as const).map((k) => (
              ...
              >
                {k === 'profile' ? 'Profile' : k === 'agents' ? 'Agents' : k === 'positions' ? 'Positions' : 'Themes'}
              </button>
            ))}
          </div>
        </div>
                      ...
                      <div key={a.id} className="px-4 py-3 border-t border-white/10 flex items-center justify-between">
                        <div>
                          <div className="text-sm font-semibold">
                            ...
                            {a.is_agency_owner ? <span className="ml-2 text-[10px] px-2 py-1 rounded-xl border bg-white/5 border-white/10 text-white/70">Owner</span> : null}
                            {a.role === 'admin' ? <span className="ml-2 text-[10px] px-2 py-1 rounded-xl border bg-white/5 border-white/10 text-white/70">Admin</span> : null}
                          </div>
        ...
        {tab === 'positions' && (
          <div className="glass rounded-2xl border border-white/10 p-6">
            ...
            <div className="text-xs text-white/55 mt-1">Comp is in 5% increments. Effective date uses glass calendar.</div>
